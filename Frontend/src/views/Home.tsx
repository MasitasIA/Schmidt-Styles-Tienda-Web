        ...
        const fetchRecientes = async () => {
            try {
                const { data, error } = await supabase
                    .from('productos')
                    .select('*, imagenes:imagenes_producto(id, imagen), categorias (id, nombre)')
                    .eq('activo', true)
        ...
        <div className="w-full bg-background text-text selection:bg-accent selection:text-text">
            ...
            <section className="relative px-4 py-24 md:py-32 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-text-secondary font-bold max-w-xl mb-12 text-lg uppercase tracking-wide">
                    ...
                    <span className="text-text">Prendas diseñadas para destacar en el asfalto.</span>
                </p>
            ...
            <section className="container mx-auto px-4 pb-24">
                <div className="flex items-end justify-between mb-8 border-b-4 border-text pb-4">
                    ...
                    <div className="hidden md:flex gap-4">
                        <button onClick={slideLeft} className="w-14 h-14 flex items-center justify-center border-4 border-text bg-background hover:bg-text hover:text-background shadow-[4px_4px_0px_0px_var(--color-text)] hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all">
                            <ArrowLeft size={28} strokeWidth={3} />
                        </button>
                        <button onClick={slideRight} className="w-14 h-14 flex items-center justify-center border-4 border-text bg-background hover:bg-text hover:text-background shadow-[4px_4px_0px_0px_var(--color-text)] hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all">
                ...
                {cargando ? (
                    <div className="w-full h-80 border-4 border-text bg-background flex items-center justify-center shadow-[8px_8px_0px_0px_var(--color-text)] animate-pulse">
                        <span className="font-black uppercase tracking-widest text-text">[ Cargando Novedades ]</span>
                    </div>
                ) : productosRecientes.length === 0 ? (
                    <div className="w-full h-80 border-4 border-dashed border-text bg-background flex items-center justify-center">
                        <span className="font-black uppercase tracking-widest text-text-secondary text-2xl">[ No hay lanzamientos recientes ]</span>
                    </div>
                ) : (
                    ...
                    <div 
                        ...
                        style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }} // Oculta la barra scroll fea nativa
                    >
                        {productosRecientes.map((producto) => (
                            <Link 
                                key={producto.id}
                                to={`/producto/${producto.id}`}
                                className="group flex flex-col min-w-[280px] sm:min-w-[320px] max-w-[320px] shrink-0 snap-start border-4 border-text bg-background hover:shadow-[12px_12px_0px_0px_var(--color-accent)] transition-all duration-300"
                            >
                                ...
                                <div className="p-5 flex flex-col flex-1">
                                    ...
                                    <div className="mt-auto pt-4 border-t-2 border-text flex justify-between items-center">
                                        ...
                                        <ArrowRight size={20} strokeWidth={3} className="text-text group-hover:text-accent group-hover:translate-x-1 transition-all" />
                                    </div>
                                </div>
                            </Link>
                        ))}
                ...
                <Link to="/catalogo" className="mt-8 md:hidden flex items-center justify-center gap-2 border-4 border-text py-4 font-black uppercase tracking-widest text-text hover:bg-text hover:text-background transition-colors shadow-[4px_4px_0px_0px_var(--color-text)]">
                    Ver todo el catálogo <ArrowRight size={24} strokeWidth={3} />
                </Link>
            </section>
            ...
            <section className="container mx-auto px-4 pb-24">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div className="group border-2 border-primary-hover bg-primary p-8 flex flex-col items-start text-left hover:border-secondary hover:shadow-[8px_8px_0px_0px_var(--color-secondary)] transition-all duration-300">
                        <Truck size={40} className="mb-6 text-text group-hover:text-accent transition-colors" />
                        ...
                        <p className="text-text-secondary font-medium text-sm leading-relaxed">
                            Despachamos tu pedido rápidamente para que llegue a tu casa sin demoras.
                        </p>
                    </div>
                    {/* Card 2 */}
                    <div className="group border-2 border-primary-hover bg-primary p-8 flex flex-col items-start text-left hover:border-secondary hover:shadow-[8px_8px_0px_0px_var(--color-secondary)] transition-all duration-300">
                        <ShieldCheck size={40} className="mb-6 text-text group-hover:text-accent transition-colors" />
                        <h3 className="text-xl font-black uppercase tracking-widest mb-3 text-text">
                            Compra segura
                        </h3>
                        <p className="text-text-secondary font-medium text-sm leading-relaxed">
                            Pagos protegidos y múltiples medios de pago disponibles a través de plataformas oficiales.
                        </p>
                    </div>
                    {/* Card 3 */}
                    <div className="group border-2 border-primary-hover bg-primary p-8 flex flex-col items-start text-left hover:border-secondary hover:shadow-[8px_8px_0px_0px_var(--color-secondary)] transition-all duration-300">
                        <Zap size={40} className="mb-6 text-text group-hover:text-accent transition-colors" />
                        <h3 className="text-xl font-black uppercase tracking-widest mb-3 text-text">
                            Calidad premium
                        </h3>
                        <p className="text-text-secondary font-medium text-sm leading-relaxed">
                            Materiales pesados y seleccionados minuciosamente para una durabilidad superior en la calle.
                        </p>
                    </div>
                </div>
            </section>
